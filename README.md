...
#### Ёмкий пример
    (Из статьи на хабре в `cmd/some_habr_test`)
В этом примере 2 уровня вложенности: на каждом уровне вложенности симулируется
трудоёмкая задача с помощью задержки в милисекундах. 